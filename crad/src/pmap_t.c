...
void alloc_pmap(pmap_t *pmap) {
  pmap->p =
      (param_pair_t *)realloc(pmap->p, (pmap->n + 1) * sizeof(param_pair_t));
...
const char *pmap_gkey(const pmap_t *pmap, int i) { return pmap->p[i].key; }
...
const char *pmap_gvalue(const pmap_t *pmap, int i) { return pmap->p[i].value; }
