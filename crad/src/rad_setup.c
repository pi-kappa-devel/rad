...
double linterpV12d(
	const sol_t* s, short x1, short r1, double xp, double rp,
	const setup_t* u
) {
	short x2 = x1 + 1;
	short r2 = r1 + 1;
...
#ifdef RAD_DEBUG
		if(td->vM < td->v0buf[td->li]) td->vM = td->v0buf[td->li];
#endif
	}
}
...
void step_sovle(thread_init_t* td) {
	...
	for(td->li = 0; td->li < td->u->c->w[td->wid].l.s; ++td->li) {
		...
		if(td->acc < diff) td->acc = diff;
		if(td->qM < td->qpolbuf[td->li]) td->qM = td->qpolbuf[td->li];
		if(td->sM < td->spolbuf[td->li]) td->sM = td->spolbuf[td->li];
		if(td->vM < td->v0buf[td->li]) td->vM = td->v0buf[td->li];
	}
}
...
void copybufs(thread_init_t* td) {
	...
	if(td->u->c->accbuf < td->acc) td->u->c->accbuf = td->acc;
	if(td->u->c->sMbuf < td->sM) td->u->c->sMbuf = td->sM;
	if(td->u->c->qMbuf < td->qM) td->u->c->qMbuf = td->qM;
	if(td->u->c->vMbuf < td->vM) td->u->c->vMbuf = td->vM;
}
...
void init_pipeline(setup_t* u) {
	...
	for(int i=1; i<=RAD_NUM_THREADS; ++i) {
		...
		u->c->w[i-1].l.s = (i<rem)?
			u->c->w[RAD_NUM_THREADS].l.s + 1 : u->c->w[RAD_NUM_THREADS].l.s;
		// Current worker's logical offset = previous worker's logical end
		u->c->w[i].l.o = u->c->w[i-1].l.e = u->c->w[i-1].l.o + u->c->w[i-1].l.s;
...
/** @brief Save setup
 * @details Consolidates model and solution saving functionality. The format and naming conventions of the saved
 * binary data are set in the model_save() and solution_save() functions.
...
void create_thread(thread_init_t* td, int i, thead_init_callback thread_main) {
	...
	status =  pthread_create(
		&td->u->c->w[i].thread, &td->u->c->attr, thread_main, td
	);
#else
	td->u->c->w[i].thread = CreateThread(
		NULL, 0, thread_main, (PVOID)td, 0, NULL
	);
...
void resume_concurrency(setup_t* u) {
	...
	for(int xi=0; xi<u->s->xg->n; ++xi) {
		for(int ri=0; ri<u->s->rg->n; ++ri) {
			if(u->c->qMbuf<u->s->qpol[xi][ri]) u->c->qMbuf = u->s->qpol[xi][ri];
			if(u->c->sMbuf<u->s->spol[xi][ri]) u->c->sMbuf = u->s->spol[xi][ri];
			if(u->c->vMbuf<u->s->v1[xi][ri]) u->c->vMbuf = u->s->v1[xi][ri];
		}
	}
...
/** @brief Load setup
 * @details Consolidates model and solution loading functionality. The format and naming conventions of the binary
 * data are set in the model_load() and solution_load() functions. The order of the functions in the model's
 * Inherits the convention used in model loading.
...
int setup_init(setup_t* u, const char* parameter_filename, const struct objpart_st* obhparts) {
	...
	snprintf(pfile_path, RAD_PATH_BUFFER_SZ, "%s" CCM_FILE_SYSTEM_SEP  "%s", RAD_DATA_DIR, parameter_filename);
...
void main_sync(thread_init_t* td) {
	...
	td->u->c->vMbuf = 0;
		...
		if(td->u->s->it && td->u->s->it%RAD_SAVE_CYCLE==0) {
			...
			snprintf(
				buf, RAD_PATH_BUFFER_SZ,
				"save" CCM_FILE_SYSTEM_SEP  "it%05d", td->u->s->it
			);
...
/** @brief Model solver
 * @details This is the top-level main functionality call. The function expects an
 * initialized model setup (see setup_init()). If multi-threading mode is
...
int setup_solve(setup_t* u) {
	...
	thread_init_t td = {
		.wid = RAD_NUM_THREADS,.u = u,.pv0 = u->s->v0,
		.ovar = {.m = u->m}
	};
	alloc_thread_init(&td);
 ...
 * @details The functionality is similar to setup_solve(). The function is intended to be used for resuming execution
 * from a point stored in the file system. This functionality is helpful when the application is executed in
 * environments with execution-time constraints. It assumes that the passed setup is populated using the setup_load()
 * function. In contrast to setup_solve(), this function does not call setup initialization routines. It rather
 * jumps directly to iteration functionality execution.
 * @param u Model setup
 * @return Zero on success, non-zero otherwise */
 ...
 * @details This is an auxiliary function that located the last setup save point in a particular folder. It is
 * intended to be used in conjunction with the periodic iteration backup functionality of the applications. When
 * RAD_SAVE_CYCLE is positive, the applications create backup binary files of the execution status in the file system
 * every RAD_SAVE_CYCLE iterations. The files are stored in directories of the form ´itN`, where N is replaced by the
 * iteration count. This function searches into the applications' data path for the greatest saved iteration. It
 * stores the located path into the passed character pointer.
...
int setup_find_last_saved(char* save_point) {
	...
	snprintf(dir_path, RAD_PATH_BUFFER_SZ, "%s" CCM_FILE_SYSTEM_SEP  "save", RAD_TEMP_DIR);
	if ((dir = opendir(dir_path)) != NULL) {
		...
		if (last) {
			snprintf(
				save_point, RAD_PATH_BUFFER_SZ,
				"save" CCM_FILE_SYSTEM_SEP  "%s", last->d_name
			);
		}
		else {
			return -1;
	...
	snprintf(dir_path, RAD_PATH_BUFFER_SZ, "%s" CCM_FILE_SYSTEM_SEP  "save" CCM_FILE_SYSTEM_SEP "it*", RAD_TEMP_DIR);
	...
	snprintf(
		save_point, RAD_PATH_BUFFER_SZ, "save" CCM_FILE_SYSTEM_SEP  "%s", last
	);
	FindClose(hFind);
#endif
