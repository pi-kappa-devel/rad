...
The file contains three basic classes; namely a grid, a variable and a model class. These classes provide are used as
a bridge between the c applications and the Python Jupyter notebook. The c applications solve the radial attention
model, generate solution approximation data and store them in the file system. These classes load the data and provide
an interface for them to be used in a Python Jupyter notebook. The notebook functionality creates the tables and the
...
class Grid:
    ...
    def eqpart(self, size):
        ...
        The function returns a set of indices, the values of which approximate a grid's equipartition. The
        function calculates the values of an equipartition of the passed size for the continuous domain of the grid.
        For each calculated value, it locates the index that corresponds to the lowest upper bound of this value in
        the discretized grid's domain. The set of these indices are returned.
    ...
    def lower_bound_index(self, values):
        ...
        For each element of the passed values, it finds and returns the minimum among the indices with values that are
        greater than the value.

        Args:
            values (ndarray): Array with values the lower bound indices of which are to be located.
...
class Variable:
    ...
    Loads and holds the binary data that approximate the optimal controls and the value functions generated by
    the c applications.
    ...
    def save_figs(self, fig_data):
        ...
        Args:
            fig_data (dict): Figure data. Allowed keys are {angle, prefix, zlabel, rsections, rsections_points,
        ...
        plt.show()
        fig.savefig(
            rad_conf.RAD_TEMP_DIR + "/" + fig_data["prefix"] + "_surf.pdf",
            dpi="figure", format="pdf", transparent=True, bbox_inches="tight", pad_inches=0.3
        )
        ...
        for r_idx, r_val in enumerate(fig_data["rsections"]):
            ...
            if "rsections_points" in fig_data:
                plt.plot(
                    fig_data["rsections_points"][r_idx][1][0], fig_data["rsections_points"][r_idx][1][1],
                    linestyle="None", marker=".", color="black", markersize=5
                )
                axes.annotate(
                    fig_data["rsections_points"][r_idx][0], fig_data["rsections_points"][r_idx][1],
                    xytext=(-18, 12), va="top", textcoords="offset points"
                )
        plt.legend(loc="best")
        plt.show()
        fig.savefig(
            rad_conf.RAD_TEMP_DIR + "/" + fig_data["prefix"] + "_rsections.pdf",
            dpi="figure", format="pdf", transparent=True, bbox_inches="tight", pad_inches=0.3
        )
        ...
        for x_idx, x_val in enumerate(fig_data["xsections"]):
            ...
            if "xsections_points" in fig_data:
                plt.plot(
                    fig_data["xsections_points"][x_idx][1][0], fig_data["xsections_points"][x_idx][1][1],
                    linestyle="None", marker=".", color="black", markersize=5
                )
                axes.annotate(
                    fig_data["xsections_points"][x_idx][0], fig_data["xsections_points"][x_idx][1],
                    xytext=(-10, 12), va="top", textcoords="offset points"
                )
        plt.legend(loc="best")
        plt.show()
        fig.savefig(
            rad_conf.RAD_TEMP_DIR + "/" + fig_data["prefix"] + "_xsections.pdf",
            dpi="figure", format="pdf", transparent=True, bbox_inches="tight", pad_inches=0.5
        )
...
class RadialAttentionModel:
    ...
    Loads binary data of the c setup structure. The format of the binary data is specified in the documentation of the
    c applications.
...
\multirow{ 2}{*}{quantity} & \multirow{ 2}{*}{$qn} & \multirow{ 2}{*}{$qmin} & $qmax & \multirow{ 2}{*}{$qc} \\
 &  & & (120000) &  \\\hdashline
\multirow{ 2}{*}{effort} & \multirow{ 2}{*}{$sn} & \multirow{ 2}{*}{$smin} & $smax & \multirow{ 2}{*}{$sc} \\
...
complementarity factor ($$\gamma$$) & $gamma \\\hdashline
...
\end{subtable}"""
    ...
    def __parse_fnc__(self, spec):
        ...
        output_str = output_str.replace("v->m->alpha", "{}".format(self.parameters["alpha"]))
        output_str = output_str.replace("v->m->gamma", "{}".format(self.parameters["gamma"]))
        output_str = output_str.replace("v->m->delta", "{}".format(self.parameters["delta"]))
    ...
    def __init__(self, save_dir):
        ...
        self.grids["x"] = Grid(self.data_path + "/xg")
        self.grids["r"] = Grid(self.data_path + "/rg")
        self.grids["q"] = Grid(self.data_path + "/qg")
        self.grids["s"] = Grid(self.data_path + "/sg")

        self.variables["v0"] = Variable(self.grids["x"], self.grids["r"], datafile=self.data_path + "/v0")
        self.variables["v1"] = Variable(self.grids["x"], self.grids["r"], datafile=self.data_path + "/v1")
        self.variables["s"] = Variable(self.grids["x"], self.grids["r"], datafile=self.data_path + "/spol")
        self.variables["q"] = Variable(self.grids["x"], self.grids["r"], datafile=self.data_path + "/qpol")
        ...
        with open(self.data_path + "/fncs", "r") as file_handle:
            while True:
                ...
                if line.startswith("util"):
                    ...
                    self.specification["util"]["fnc"] = eval("lambda q, r, s: " + self.specification["util"]["str"])
                elif line.startswith("cost"):
                    self.specification["cost"] = {"str": self.__parse_fnc__(line[:-1])}
                    self.specification["cost"]["fnc"] = eval("lambda r, s: " + self.specification["cost"]["str"])
                elif line.startswith("radt"):
                    self.specification["radt"] = {"str": self.__parse_fnc__(line[:-1])}
                    self.specification["radt"]["fnc"] = eval("lambda r, s: " + self.specification["radt"]["str"])
                elif line.startswith("wltt"):
                    self.specification["wltt"] = {"str": self.__parse_fnc__(line[:-1])}
                    self.specification["wltt"]["fnc"] = eval("lambda q, r, s, x: " + self.specification["wltt"]["str"])

    def model_sting(self):
        """Get a brace-nested, string description of the model object."""

        mask = "model={{.alpha={}, .beta={}, .delta={}, .gamma={}, .R={}, .util={}, .cost={}, .radt={}, .wltt={}}}"
        return mask.format(
            self.parameters["alpha"], self.parameters["beta"], self.parameters["delta"], self.parameters["gamma"],
            self.parameters["R"],
            self.specification["util"]["str"], self.specification["cost"]["str"],
            self.specification["radt"]["str"], self.specification["wltt"]["str"]
        )
    ...
    def get_radius_dynamics(self):
        ...
        for x_idx in range(0, len(self.grids["x"].data)):
            for r_idx, r_val in enumerate(self.grids["r"].data):
                radt[r_idx, x_idx] = self.specification["radt"]["fnc"](r_val, self.variables["s"].data[r_idx, x_idx])
        ...
        for x_idx, x_val in enumerate(self.grids["x"].data):
            for r_idx, r_val in enumerate(self.grids["r"].data):
                wltt[r_idx, x_idx] = self.specification["wltt"]["fnc"](
                    self.variables["q"].data[r_idx, x_idx], r_val, self.variables["s"].data[r_idx, x_idx], x_val
                )
    ...
    def save_latex_table(self, filename):
        ...
        with open(rad_conf.RAD_TEMP_DIR + "/" + filename, "w") as file_handle:
            file_handle.write(Template(self.__ltbl_prototype__).substitute(
                x_size=len(self.grids["x"].data), xmin="{:g}".format(self.grids["x"].data[0]),
                xmax="{:g}".format(self.grids["x"].data[-1]), xc="{:g}".format(self.grids["x"].weight),
                r_size=len(self.grids["r"].data), rmin="{:g}".format(self.grids["r"].data[0]),
                rmax="{:g}".format(self.grids["r"].data[-1]), rc="{:g}".format(self.grids["r"].weight),
                qn=len(self.grids["q"].data), qmin="{:g}".format(self.grids["q"].data[0]),
                qmax="{:g}".format(self.grids["q"].data[-1]), qc="{:g}".format(self.grids["q"].weight),
                sn=len(self.grids["s"].data), smin="{:g}".format(self.grids["s"].data[0]),
                smax="{:g}".format(self.grids["s"].data[-1]), sc="{:g}".format(self.grids["s"].weight),
                beta=self.parameters["beta"], delta=self.parameters["delta"], alpha=self.parameters["alpha"],
                gamma=self.parameters["gamma"]
            ))

    def save_learning_rsections(self, rsections=None):
        """Create. plot and save as pdf the learning dynamics' r sections."""
        ...
        plt.show()
        fig.savefig(
            rad_conf.RAD_TEMP_DIR + "/l_rsections.pdf",
            dpi="figure", format="pdf", transparent=True, bbox_inches="tight", pad_inches=0.3
        )

    def save_cost_rsections(self, rsections=None):
        """Create. plot and save as pdf the cost functions' r sections."""
        ...
        plt.show()
        fig.savefig(
            rad_conf.RAD_TEMP_DIR + "/c_rsections.pdf",
            dpi="figure", format="pdf", transparent=True, bbox_inches="tight", pad_inches=0.3
        )
...
class ParameterDependence:
    ...
    def __init__(self, parameter_string):
        ...
        self.data_files = [file.replace(rad_conf.RAD_DATA_DIR + "/", "") for file in glob.glob(
            rad_conf.RAD_DATA_DIR + "/{}/{}*".format(self.parameter_string, self.parameter_string)
        )]
        self.data_files.sort()

    def save_fig(self, domain_data, ylab, yval):
        """Create, show and save in pdf format the figures with the responses of the value function and the optimal
        controls on the particular parameter described by the passed the domain data.
        """
        ...
        for r_idx in range(0, len(domain_data["r_indices"])):
            for x_idx in range(0, len(domain_data["x_indices"])):
                ...
                axes.plot(
                    domain_data["param_grid"], smoothed,
                    label="$r_{}={:.2f},x_{}={:.2f}$".format(
                        r_idx, domain_data["r_points"][g_idx], x_idx, domain_data["x_points"][g_idx]
                    )
                )
        plt.legend(loc="best")
        plt.show()
        fig.savefig(
            rad_conf.RAD_TEMP_DIR + "/{}_on_{}.pdf".format(self.parameter_string, ylab),
            dpi="figure", format="pdf", transparent=True, bbox_inches="tight", pad_inches=0.3
        )

    def save_figs(self, r_indices=None, x_indices=None):
        """Create, show and save in pdf format the figures with the responses of the value function and the optimal
        controls on value changes of each parameter.
        """
        ...
        for p_idx, p_val in enumerate(self.data_files):
            ...
            for r_idx, r_val in enumerate(r_indices):
                for x_idx, x_val in enumerate(x_indices):
                    ...
                    range_data["spol"][g_idx, p_idx] = model.variables["s"].data[r_val, x_val]
                    range_data["qpol"][g_idx, p_idx] = model.variables["q"].data[r_val, x_val]
                    range_data["v"][g_idx, p_idx] = model.variables["v0"].data[r_val, x_val]
